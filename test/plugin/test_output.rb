...
module FluentPluginOutputTest
  ...
  class DummyFullFeatureOutput < Fluent::Plugin::Output
    attr_accessor :prefer_buffered, :prefer_delayed
    def initialize
      super
...
class OutputTest < Test::Unit::TestCase
  sub_test_case 'basic output feature' do
    ...
    test '#metadata returns object which contains tag/timekey/variables from records as specified in configuration'
    test '#emit calls #emit_sync for non-buffered output'
    test '#emit calls #emit_buffered for buffered output'
    test '#prefer_buffered_processing decides buffered or not without <buffer> section'
    test '#prefer_delayed_commit decides delayed commit is enabled or not if both are implemented'
  end
